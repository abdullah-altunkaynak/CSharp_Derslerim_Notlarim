...
namespace Ders_7_
{
    ...
    /*
     Üç eğik çizgi ile gösterilir ve her XML yorum satırı için bu üç çizgiyi koymamız gerekir.
     Komut satırından .NET Core ile bir uygulama geliştiriyorsak, GenerateDocumentationFile <PropertyGroup> .csproj proje dosyamızın bölümüne
    öğesi olarak ekleyebiliriz. Ayrıca, belge dosyasının yolunu doğrudan DocumentationFile öğesini kullanarak belirtebiliriz. Aşağıdaki örnek,
    derleme ile aynı kök dosya adına sahip proje dizininde bir XML dosyası oluşturur:
    <GenerateDocumentationFile>true</GenerateDocumentationFile>
    XML yorum satırlarında yukarıdaki örnekte görüldüğü gibi etiketler kullanılır. Etiketler ve işlevlerini kısaca gösterelim.
    <summary> : Bu etiket, bir tür veya üye hakkında kısa bilgiler ekler. <summary>Etiketi önemlidir ve içeriği, IntelliSense'de veya
    BIR API başvuru belgesinde bulunan içerik veya üye bilgilerinin birincil kaynağıdır.
    <remarks> : Bu etiket, <summary>etiketinin sağladığı türler veya üyeler hakkındaki bilgileri tamamlar.
    <returns> : Bu etiket, bir yöntem bildiriminin dönüş değerini açıklar.
    <value>   : Bu etiket, <returns> etiketiyle benzerdir, ancak bunu özellikler için kullanmanız gerekir.
    <example> : XML belgelerinize bir örnek eklemek için bu etiketi kullanın.
    <para>    : İçeriğin üst etiketi içinde biçimlendirmek için bu etiketi kullanın. <para> genellikle <returns> metin paragraflarına
    bölmek için veya <remarks> gibi bir etiket içinde kullanılır.
    <c>       : Biçimlendirme konusunda hala, <c> metnin bir kısmını kod olarak işaretlemek için kullanılır.
    <exception> : <exception>Etiketini kullanarak, geliştiricilerin bir yöntemin belirli özel durumlar oluşturduğunu bilmesini sağlar.
    <see>       : <see>Etiketi, başka bir kod öğesi için bir belge sayfasına tıklatılabilir bir bağlantı oluşturmanıza olanak sağlar.
    <seealso>   : <seealso>Etiketi, <see> etiketi yaptığınız şekilde kullanırsınız. Tek fark, içeriğinin genellikle bir "Ayrıca bkz."
    bölümüne yerleştirilme nedendir.
    <param>     : <param>Bir yöntemin parametrelerini anlatmak için etiketini kullanırsınız.
    <typeparam> : <typeparam> tıpkı <param> etiketi gibi kullanırsınız, ancak genel tür veya yöntem bildirimlerinin
    genel bir parametreyi tanımlamasını sağlar. 
    <paramref>  : Bir parametreye başvuru yapmak için.
    <typeparamref> : Başvuru şeklinde genel tür veya yöntem bildirimlerinin genel bir parametreyi tanımlamasını sağlar.
    <list>      : <list>Belge bilgilerini sıralı liste, sırasız liste veya tablo olarak biçimlendirmek için etiketini kullanın.
    <inheritdoc> : <inheritdoc>Temel sınıflardan, arabirimlerden ve benzer metotlardan XML açıklamalarını devralacak şekilde etiketini
    kullanabilirsiniz. Bu, yinelenen XML açıklamalarını istenmeyen kopyalama ve yapıştırmayı ortadan kaldırır ve otomatik olarak XML
    açıklamalarını eşitlenmiş halde tutar.
    <include>   : <include>Etiketi, belge açıklamalarını doğrudan kaynak kodu dosyanıza yerleştirmeyi aksine, kaynak kodunuzda türleri
    ve üyeleri tanımlayan ayrı bır XML dosyasındaki açıklamalara başvurmanıza olanak sağlar.

    Yukarıdakilerin hepsi c# tarafından tanınan etiketlerdir, kişi isterse kendi etiketini tanımlayabilir. Sandrole gibi araçlar tanınmayak başka
    etiketleri derleyicimizin tanımasını sağlar veya bizim kendimiz için etiketler tanımlamamızı ve c# derleyicisine tanıtmamızı sağlar.
     */
    #endregion
}
